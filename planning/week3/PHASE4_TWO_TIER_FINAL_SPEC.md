...
**Based on**: BFO/CCO Expert Review + Gap Resolution
...
| Decision | Resolution |
|----------|------------|
| Act Actuality | Option B: Instantiate with `actualityStatus: "prescribed"` |
| Dormant Roles | Keep roles with `rdfs:comment` indicating dormancy |
| Scarcity Modeling | Use `cco:is_about` linking ICE to real entities |
...
|------|-------------|--------------|
| `cco:InformationBearingEntity` | The physical/digital document | GDC |
| `tagteam:DiscourseReferent` | A mention in text referring to something | ICE |
...
| `tagteam:ValueAssertionEvent` | Detected ethical value | Occurrent |
| `tagteam:ContextAssessmentEvent` | Context intensity assessment | Occurrent |
...
- Includes GIT-Minimal metadata (`assertionType`, `validInContext`)
...
- Roles inhere in Persons via `bfo:inheres_in`
...
| `bfo:inheres_in` | Role | Person | Tier 2 only | Role depends on bearer |
...
### 3.2 Resolution: Actuality Qualifier

All Tier 2 acts include `tagteam:actualityStatus` to indicate their temporal/modal status:
...
|---------------|--------------|----------------|
| `cco:AgentRole` | `cco:Person` | DiscourseReferent, Artifact |
| `cco:PatientRole` | `cco:Person` | DiscourseReferent, Artifact |
| `cco:BeneficiaryRole` | `cco:Person` | DiscourseReferent, Artifact |
...
Scarcity is a **contextual assessment**, not an intrinsic property. Model it as an Information Content Entity:
...
All Tier 1 ICE nodes link back to the IBE:
...
Every Tier 1 ICE and assertion event must include:
...
## 11. SHACL Validation Shapes

### 11.1 Cross-Tier Link Validation
...
### 11.2 Role Bearer Validation

```turtle
# Role must inhere in Person, not DiscourseReferent
tagteam:RoleBearerShape a sh:NodeShape ;
  ...
  sh:property [
    ...
    sh:minCount 1 ;
    sh:class cco:Person ;
    sh:message "Role must inhere in cco:Person, not DiscourseReferent" ;
  ] .
```

### 11.3 Act Agent Validation
...
| `RoleDetector.js` | **Major** | Roles inhere in Persons; use `inheres_in` only |
...
### 12.3 Acceptance Criteria Updates
...
ScarcityAssertion ─is_about──► Artifact, Person
```
